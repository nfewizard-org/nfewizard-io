...
export class NFCERecepcaoEventoService extends BaseNFE implements NFERecepcaoEventoServiceImpl {
    tpEvento: string;
    modelo?: string;
    ...
    constructor(environment: Environment, utility: Utility, xmlBuilder: XmlBuilder, axios: AxiosInstance, saveFiles: SaveFilesImpl, gerarConsulta: GerarConsultaImpl) {
        ...
        console.log('constructor NFCERecepcaoEventoService');
    }
    ...
    private isAmbienteNacional(tpEvento: string) {
        // Para NFCe, cancelamento (110111) é regional
        switch (tpEvento) {
            case '110111':
                return false;
            default:
    ...
    private separaEventosPorAmbiente(evento: TipoEvento[]) {
        logger.info('Dividindo eventos por ambiente', {
            context: 'NFCERecepcaoEventoService',
        });
        // Para NFCe, todos os eventos são regionais
        const nacional: TipoEvento[] = [];
        const regional = evento;
    ...
    private trataRetorno(responseInJson: GenericObject) {
        logger.info('Tratando retorno dos eventos NFCe', {
            context: 'NFCERecepcaoEventoService',
        });
    ...
    protected async callWebService(xmlConsulta: string, webServiceUrl: string, ContentType: string, action: string, agent: Agent): Promise<AxiosResponse<any, any>> {
        ...
        logger.http('Iniciando comunicação com o webservice NFCe', {
            context: `NFCERecepcaoEventoService`,
            method: this.metodo,
            url: webServiceUrl,
            action,
        ...
        logger.http('Comunicação NFCe concluída com sucesso', {
            context: `NFCERecepcaoEventoService`,
            method: this.metodo,
...
export default NFCERecepcaoEventoService;
