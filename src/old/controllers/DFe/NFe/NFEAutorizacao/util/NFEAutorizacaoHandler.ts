...
class NFEAutorizacaoHandler {
    ...
    public validarCpfCnpj(cpfCnpj: string): { documentoValido: boolean, tipoDoDocumento: 'CPF' | 'CNPJ' | 'Desconhecido' } {
        ...
        if (isCpf) {
            ...
            if (!this.documentoValidateCpf(cleanCpfCnpj)) {
                return { documentoValido: false, tipoDoDocumento: 'CPF' };
            }
            ...
            // CNPJ
            if (!this.documentoValidateCnpj(cleanCpfCnpj)) {
     ...
     */
    private documentoValidateCpf(cpf: string): boolean {
        let sum = 0;
        let remainder;
     ...
     */
    private documentoValidateCnpj(cnpj: string): boolean {
