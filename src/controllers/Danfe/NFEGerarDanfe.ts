...
class NFEGerarDanfe {
    ...
    _buildDestinatario() {
        const { top, left } = this.doc.page.margins;
        const docDest = this.documento.mascaraCnpjCpf(this.dest.CNPJCPF?.toString() || '')
        this.setLineStyle(0.75, '#1c1c1c');
    ...
    _builTransporte() {
        ...
        const _buildCalcImposto = () => {
            const CNPJCPF = this.transp.transporta?.CNPJCPF?.toString()
            const CPF = this.transp.transporta?.CPF?.toString()
            const documento = this.documento.mascaraCnpjCpf(CNPJCPF || CPF || '');
